...
%>
<% 
	String acnum=(String)request.getParameter("searchAccountNumber");
	String name=(String)request.getParameter("searchName");
	String userId=(String)request.getParameter("searchUserId");
%>
<!DOCTYPE html>
<html>
<head>
...
<body>
	...
	<div class="container my-2">
		<h3 class="display-4 text-center"> Customer Details </h3>
	</div>\
			...
			<div class="col-md-12">
				<div class="form-group form-inline">
					<form action="processSearch.jsp" method="post">
						<input type="text" name="searchAccountNumber" class="form-control form-inline"  placeholder="Account Number" autocomplete="off">
						<input type="text" name="searchName" class="form-control form-inline"  placeholder="Name" autocomplete="off">
						<input type="text" name="searchUserId" class="form-control form-inline"  placeholder="User ID" autocomplete="off">
	...
	<div class="container mt-3 py-4">
		...
		<table class="table  table-bordered table-hover table-striped">
			...
			<%
				List<CustomerStructure> li=Customer.search(acnum,name,userid);
				for(int i=0;i<li.size();i++)
				{
			%>
			<tr>
				<td> <% out.println( li.get(i).getCustomerAccountNumber()); %></td>
				<td> <% out.println(li.get(i).getName()); %></td>
				<td> <% out.println(li.get(i).getEmail()); %></td>
				<td><% out.println(li.get(i).getUserid()); %></td>
				<td> <% out.println(li.get(i).getPassword()); %></td>
				<td> <% out.println(li.get(i).getContact()); %></td>
				<td> <% out.println(li.get(i).getAddress()); %></td>
				<td> <% out.println(li.get(i).getBalance()); %></td>
				<td> <a href="showTransitions.jsp?customerAccountNumber=<%=li.get(i).getCustomerAccountNumber()  %>"> View Transitions </a> </td>
			</tr>
			<%} %>
		</table>	
		</div>
